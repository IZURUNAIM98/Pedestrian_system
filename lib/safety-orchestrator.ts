...
export interface SafetyInputs {
  ...
  aiConfidence?: number;
  independentFallbackVerified?: boolean;
...
export function resolveSafetyDecision(input: SafetyInputs): SafetyDecision {
  ...
  if (input.pedestrianDemand) {
    if (sensorState === "degraded" || confidence < 0.75) {
      if (input.independentFallbackVerified) {
        return decision("PEDESTRIAN_SERVICE", "Camera input is degraded, but the authenticated request and independent radar, LiDAR, stop-line, vehicle-presence and controller checks verified a fixed protected crossing.", {
          pedestrian: "WALK", phaseClock: "walk", rejectedCommands: ["camera-only WALK", "conflicting vehicle GREEN"], requiresHumanReview: true,
        });
      }
